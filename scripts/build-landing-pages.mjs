...
// lastmod must mean "this page changed", not "a build ran". The generator now
// runs on every `npm run build`, so stamping today unconditionally would churn
// all four dates on every deploy and teach Google to ignore lastmod entirely.
// Keep the previous date unless the generated bytes actually moved.
const smPath = join(ROOT, 'public', 'sitemap.xml');
// Normalise to LF on read. On Windows the checked-out file has CRLF, while the
// strip regex below anchors on a newline and would leave orphan carriage returns
// behind — emitting a mixed-ending file that reads as a whole-file diff every build.
const smPrev = readFileSync(smPath, 'utf8').replace(/\r\n?/g, '\n');
const prevMod = new Map(
  [...smPrev.matchAll(/<loc>[^<]*?\/([a-z-]+)\/<\/loc>\s*<lastmod>([\d-]{10})<\/lastmod>/g)]
    .map((m) => [m[1], m[2]])
);
...
for (const p of PAGES) {
  ...
  const html = page(p);
  const unchanged = existsSync(file) && readFileSync(file, 'utf8') === html;
  p.lastmod = unchanged ? (prevMod.get(p.slug) ?? today) : today;
  mkdirSync(dir, { recursive: true });
  writeFileSync(file, html, 'utf8');
  console.log(`  /${p.slug}/${unchanged ? '' : '   (changed — lastmod bumped)'}`);
}

// ---- rebuild sitemap with every URL ----
let sm = smPrev.replace(
...
if (!qa.length) {
  ...
  // Splice by index, not by RegExp. In a template literal, JS resolves `\s` and
  // `\S` as string escapes long before RegExp sees them, so `[\s\S]` silently
  // compiles to `[sS]`, never spans the block, and the replace becomes a
  // permanent no-op — the schema freezes while the build still prints success.
  // indexOf/slice cannot fail that way.
  const a = html.indexOf(MARK_A);
  const b = html.indexOf(MARK_B);
  if (a !== -1 && b !== -1 && b > a) {
    html = html.slice(0, a) + block + html.slice(b + MARK_B.length);
  } else if (a === -1 && b === -1) {
    html = html.replace('</head>', `    ${block}\n  </head>`);
  } else {
    throw new Error('index.html has a mismatched FAQ-SCHEMA marker pair — refusing to write');
  }
  writeFileSync(idxPath, html, 'utf8');

  // Read back and prove the emitted schema matches faqData.ts, rather than
  // trusting that the write did anything. This is the check that would have
  // caught the no-op above on the first run.
  const emitted = readFileSync(idxPath, 'utf8');
  const sa = emitted.indexOf(MARK_A), sb = emitted.indexOf(MARK_B);
  const inner = emitted.slice(sa, sb);
  const parsed = JSON.parse(inner.slice(inner.indexOf('{'), inner.lastIndexOf('}') + 1));
  const got = parsed.mainEntity.map((m) => [m.name, m.acceptedAnswer.text]);
  const want = qa.map(({ q, a: ans }) => [q, ans]);
  if (JSON.stringify(got) !== JSON.stringify(want)) {
    throw new Error(
      `FAQ schema drift: index.html carries ${got.length} Q&A, faqData.ts defines ${want.length} (or the text differs)`
    );
  }
  console.log(`  index.html — FAQPage schema verified in sync, ${qa.length} questions`);
}
